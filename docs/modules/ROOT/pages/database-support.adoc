...
NOTE: "Quote style" in this context refers to _identifier quoting_ — how table and column names are escaped in SQL (e.g., ``` `users` ``` or `"users"`). This is separate from string literal quoting, which always uses single quotes (`'value'`) across all databases.

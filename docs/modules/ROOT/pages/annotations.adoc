...
| `@Table` | `@Table` | Table name (`name`) and schema
...
| `@Embedded` | `@Embedded` | Embed value object (note: JPA `@Embedded` has no prefix support)
| `@Lob` | `@Lob` | Large object (CLOB/BLOB)
| `@Link(linkfield=...)` | `@JoinColumn(name=...)` | Foreign key column name
| `@SubClasses` | _(none)_ | No JPA equivalent—see note below
|===

NOTE: PojoQuery annotations always take precedence when both PojoQuery and JPA annotations are present on the same element.
...
[source,java]
----
include::example$test/examples/docs/JpaAnnotationsExample.java[tag=jpa-entity,indent=0]
----
...
[source,java]
----
include::example$test/examples/docs/JpaAnnotationsExample.java[tag=pojoquery-entity,indent=0]
----
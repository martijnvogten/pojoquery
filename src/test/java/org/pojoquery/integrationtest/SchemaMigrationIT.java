...
public class SchemaMigrationIT {
    ...
    public void testIncrementalSchemaMigration() {
        ...
        DB.runInTransaction(db, (Connection c) -> {
            ...
            Assert.assertNotNull("ID should be auto-generated", product1.id);
            
            // Verify we can query the data
            ProductV1 loaded = PojoQuery.build(ProductV1.class).findById(c, product1.id);
            Assert.assertEquals("Widget", loaded.name);
        });
        ...
        Assert.assertFalse("Should generate migration statements for new columns", 
            migrationStatements.isEmpty());
        
        String migrationSql = String.join("\n", migrationStatements);
        System.out.println("V1 -> V2 Migration SQL:\n" + migrationSql);
        Assert.assertTrue("Should add description column", 
            migrationSql.toLowerCase().contains("description"));
        Assert.assertTrue("Should add price column", 
            migrationSql.toLowerCase().contains("price"));
        ...
        DB.runInTransaction(db, (Connection c) -> {
            ...
            Assert.assertEquals("Gadget", loaded.name);
            Assert.assertEquals("A useful gadget", loaded.description);
            Assert.assertEquals(29.99, loaded.price, 0.001);
            ...
            Assert.assertEquals("Widget", updated.name);
            Assert.assertEquals("The original widget", updated.description);
            Assert.assertEquals(19.99, updated.price, 0.001);
        });
        ...
        Assert.assertFalse("Should generate migration statements for V3 columns", 
            migrationStatements2.isEmpty());
        
        String migrationSql2 = String.join("\n", migrationStatements2);
        System.out.println("V2 -> V3 Migration SQL:\n" + migrationSql2);
        Assert.assertTrue("Should add stockQuantity column", 
            migrationSql2.toLowerCase().contains("stockquantity"));
        Assert.assertTrue("Should add category column", 
            migrationSql2.toLowerCase().contains("category"));
        ...
        DB.runInTransaction(db, (Connection c) -> {
            ...
            Assert.assertEquals("Super Widget", superWidget.name);
            Assert.assertEquals("The best widget ever", superWidget.description);
            Assert.assertEquals(99.99, superWidget.price, 0.001);
            Assert.assertEquals(Integer.valueOf(100), superWidget.stockQuantity);
            Assert.assertEquals("Electronics", superWidget.category);
        });
        ...
        Assert.assertFalse("Should generate migration statements for OrderItem table", 
            migrationStatements3.isEmpty());
        
        String migrationSql3 = String.join("\n", migrationStatements3);
        System.out.println("V3 + OrderItem Migration SQL:\n" + migrationSql3);
        Assert.assertTrue("Should create order_item table", 
            migrationSql3.toLowerCase().contains("order_item"));
        Assert.assertTrue("Should have productId column", 
            migrationSql3.toLowerCase().contains("productid"));
        Assert.assertTrue("Should have quantity column", 
            migrationSql3.toLowerCase().contains("quantity"));
        ...
        DB.runInTransaction(db, (Connection c) -> {
            ...
            Assert.assertEquals(Long.valueOf(1L), loadedItem1.productId);
            Assert.assertEquals(Integer.valueOf(5), loadedItem1.quantity);
            Assert.assertEquals(19.99, loadedItem1.unitPrice, 0.001);
    ...
    public void testNoMigrationNeededWhenSchemaUpToDate() {
        ...
        for (String statement : migrationStatements) {
            Assert.assertFalse("Should not have ADD COLUMN when schema is up to date",
                statement.toLowerCase().contains("add column"));
        }
    }
    ...
    public void testMigrationWithRelatedEntities() {
        ...
        DB.runInTransaction(db, (Connection c) -> {
            ...
            Assert.assertEquals(Integer.valueOf(1), loadedCat.displayOrder);
            
            ItemV2 loadedItem = PojoQuery.build(ItemV2.class).findById(c, 1L);
            Assert.assertEquals("Some Author", loadedItem.author);
            Assert.assertEquals(Integer.valueOf(500), loadedItem.pageCount);
        });
    }
